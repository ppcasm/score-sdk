...
// =============================================================
void tv_printcolorex(unsigned short *fb, unsigned int x, unsigned int y, const char *text, unsigned short fg, unsigned short bg) 
{
    short xx, yy;
        ...
        for (yy = 0; yy < 16; yy++) 
        {
            for (xx = 0; xx < 8; xx++)
            {
                ...
                if (font[(*text) * 16 + yy] & (1 << (8 - xx))) {
                    // Foreground pixel
                    fb[(y * 16 + yy) * 640 + (x * 8 + xx)] = fg;
                } else {
                    // Background pixel
                    fb[(y * 16 + yy) * 640 + (x * 8 + xx)] = bg;
                }
            }
        }
        x++;
