...
bin_preloader = """const unsigned int flash_patch[] = {
...
def main():
	...
	bin_file_len = len(bin_file)

	header_file.write(bin_preloader+'\n')

	for i in range(0, int(bin_file_len), 4):
		bin_value = (bin_file[i+3] << 24) | (bin_file[i+2]<<16) | (bin_file[i+1]<<8) | bin_file[i+0]
		header_file.write('0x{0:08X},\n'.format(bin_value))
...
if __name__ == "__main__":
	if(len(sys.argv) != 3):
		print('Wrong Usage: {0} <bin_file> <header_file>'.format(sys.argv[0]))
		exit()
